...
set "OPENSSL_DLL_ARCH=%VENDOR_WINDOWS_ARCH%"
...
perl Configure %OPENSSL_TARGET% no-tests shared || exit /b 1
nmake /nologo || exit /b 1
copy /y "libssl.lib" "..\%OUTPUT_DIR%\libssl.lib" >nul || exit /b 1
copy /y "libcrypto.lib" "..\%OUTPUT_DIR%\libcrypto.lib" >nul || exit /b 1
copy /y "libssl-3-%OPENSSL_DLL_ARCH%.dll" "..\%OUTPUT_DIR%\libssl-3-%OPENSSL_DLL_ARCH%.dll" >nul || exit /b 1
copy /y "libcrypto-3-%OPENSSL_DLL_ARCH%.dll" "..\%OUTPUT_DIR%\libcrypto-3-%OPENSSL_DLL_ARCH%.dll" >nul || exit /b 1
popd || exit /b 1
